/*
 * BitTorrentInterestedMessage.java
 *
 * Created on Feb 9, 2010, 10:41:55 PM
 *
 * Description: Provides a bit torrent interested message. See http://wiki.theory.org/BitTorrentSpecification.
 *
...
/** Provides a bit torrent interested message.
 *
 * @author reed
 */
@NotThreadSafe
public class BitTorrentInterestedMessage implements BitTorrentMessage {

  /** the peer id bytes */
  private final byte[] peerIdBytes;

  /** Constructs a new BitTorrentInterestedMessage instance.
   *
   * @param peerIdBytes the peer identification bytes
   */
  public BitTorrentInterestedMessage(final byte[] peerIdBytes) {
    //Preconditions
  ...
  public String toString() {
    ...
    stringBuilder.append("[interested, peer id: ");
    try {
  ...
  public byte[] encode() {
    ...
    bytes[3] = BitTorrentConstants.BIT_TORRENT_INTERESTED_MESSAGE_LENGTH;
    bytes[4] = BitTorrentConstants.BIT_TORRENT_INTERESTED_MESSAGE_ID;
    System.arraycopy(
            peerIdBytes,  // source
            0, // source offset
            bytes, // destination
  ...
  /** Decodes the given bytes into a new BitTorrentInterestedMessage instance.
   *
   * @param bytes the given bytes
   * @return a new instance
   */
  public static BitTorrentInterestedMessage decode(final byte[] bytes) {
    //Preconditions
    ...
    System.arraycopy(
            ...
            20); // length
    return new BitTorrentInterestedMessage(peerIdBytes);
  }
